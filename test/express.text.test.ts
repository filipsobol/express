...
describe('express.text()', () => {
  ...
  it('should parse text/plain', () => new Promise(done => {
    request(this.app)
      .post('/')
  ...
  describe('with defaultCharset option', () => {
    it('should change default charset', () => new Promise(done => {
    ...
    it('should honor content-type charset', () => new Promise(done => {
      var server = createApp({ defaultCharset: 'koi8-r' })
      var test = request(server).post('/')
  ...
  describe('with type option', () => {
    ...
    describe('when ["text/html", "text/plain"]', () => {
      ...
      it('should parse "text/html"', () => new Promise(done => {
        request(this.app)
          .post('/')
          .set('Content-Type', 'text/html')
          .send('<b>tobi</b>')
      ...
      it('should ignore "text/xml"', () => new Promise(done => {
        request(this.app)
          .post('/')
  ...
  describe('charset', () => {
    ...
    it('should parse codepage charsets', () => new Promise(done => {
      var test = request(this.app).post('/')
    ...
    it('should 415 on unknown charset', () => new Promise(done => {
      var test = request(this.app).post('/')
      test.set('Content-Type', 'text/plain; charset=x-bogus')
      test.write(Buffer.from('00000000', 'hex'))
      test.expect(415, '[charset.unsupported] unsupported charset "X-BOGUS"', done)
